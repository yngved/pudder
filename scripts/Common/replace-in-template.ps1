...
# 			- {SERVER_PORT}
#
# It will substitute  the denoted values with the values found in the 'prop_file'. It will use the 'env'/'node' pair
# to find the correct value(s) in the 'prop_file'.
#
# example usage: .\replace-in-template.ps1 -env DEV1 -node NODE1 -templateFile server-template.xml -destinationFile server.xml -propertiesFile server-xml.prop
#
# Valid values
...
param($env, $node, $templateFile, $destinationFile, $propertiesFile)
...
# Filter properties on $environment and $node
$properties = Get-Content $propertiesFile | Where-Object {$_ -like "$env*" -and $_ -like "*$node*" }
...
{		
	...
	# Remove the node part of the $key
	$delimeter = $key.IndexOf(".")
	$key = $key.Substring($delimeter + 1)	
